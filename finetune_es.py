...
'''
Early Stopping版
LossがPATIENCE回連続で低くならなかった場合，その時点で最もLossの低いモデルを保存し
学習を終了
・結果は大きく変わらなかった
'''
PATIENCE = 7
...
def main():
    ...
    es = EarlyStopping(patience=PATIENCE, verbose=False, delta=0, path=f'{hparams.train.save_dir}/checkpoint.pth', trace_func=print)
    ...
    for epoch in range(hparams.train.epochs):
        ...
        es(val_loss=val_loss, model=model)
        ...
        sys.stdout.flush()
        if es.early_stop:
            modelpath_list.append(f'{hparams.train.save_dir}/checkpoint.pth')
            break
